...
- Debug 与 Release 全量测试各 260 项通过。
- AddressSanitizer 覆盖标签批量操作与健康检查相关的 45 项测试，未报告内存错误。
...
| 指标 | 中位数 | P95 | 门槛 | 结果 |
| --- | ---: | ---: | ---: | --- |
| 1000 个连接组、每组 10 个标签的匹配与状态汇总 | 5.093 ms | 18.775 ms | 100 ms | 通过 |
| 20000 条监听规则建立索引并查询 100 个成员 | 14.502 ms | 26.747 ms | 100 ms | 通过 |
| 20000 条规则集中在同一端口时建立索引 | 13.772 ms | 16.922 ms | 100 ms | 通过 |
| 100 个成员的纯调度 | 0.114 ms | 0.147 ms | 200 ms | 通过 |
| 100 个成员的假预检与批量请求 | 12.834 ms | 26.160 ms | 200 ms | 通过 |
| 注入 75 ms 慢预检时的主线程响应 | 6.388 ms | 8.060 ms | 50 ms | 通过 |
...
- 监听索引按仓库最大规则数一次预留容量，并直接遍历规则，避免为每个连接组创建临时端点数组；Host 校验复用静态非法字符集合，避免 100 个成员预检时重复构造 `CharacterSet`。标签测试套件串行执行，防止同一批性能用例互相抢占造成不可重复的墙钟 P95。
