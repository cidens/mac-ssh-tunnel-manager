...
后续桌面测试曾出现一次面板内容变暗、搜索框保留蓝色焦点且带有单字符搜索条件的状态。代码检查确认面板隐藏时会保留 SwiftUI `TextField` 的第一响应者，确认遮罩出现时也没有主动移除搜索焦点；风险确认的取消按钮同时缺少 `Esc` 快捷键。

修复措施：

- `MenuPanel.orderOut` 在隐藏面板前清除第一响应者；
- 搜索框改用显式 `FocusState` 管理；
- 删除或风险确认出现时主动退出搜索焦点；
- `Esc` 优先取消删除确认或风险确认，没有确认层时退出搜索焦点；
- 搜索文本继续保留，用户可通过“清除”按钮重置筛选。

修复后重新执行 97 项自动测试、Release 构建、签名校验和覆盖安装。用户在真实桌面环境复测后确认问题已解决。
