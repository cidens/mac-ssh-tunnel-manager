...
- 编辑器按当前片段建议最多 8 个已有标签；忽略大小写与变音符号、排除已选项，采用后保持输入状态。
- 主面板提供 3 个可替换常驻标签和带配置数量的搜索选择器；弹窗双击不会关闭主面板。
- 标签上限为 10 个、每个 16 个字符；不兼容读取 17–32 字符的旧标签。
- 10,000 标签的 Release P95：输入匹配 5.39 ms、输入预计算 17.24 ms、主面板索引 45.41 ms，均低于对应的 50/100 ms 门槛。
- Release 全量测试通过 267 项；`0.5.0` 的版本、签名、中英文资源及隔离配置实机验收均通过，未读取或启动真实隧道。
