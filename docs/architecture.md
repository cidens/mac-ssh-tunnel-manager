...
当前版本为 `0.2.0`，版本号定义在 `SSHTunnelCore/AppVersion.swift`。

## 模块结构

项目使用 SwiftPM 管理。运行代码分为两个 target：

- `SSHTunnelCore`：纯逻辑层，负责配置模型、JSON 读写、SSH 参数生成、输入校验、端口监听解析、stderr 缓冲、进程终止等待、状态统计和版本号。
- `SSHTunnelManagerApp`：macOS SwiftUI 菜单栏应用，负责表单、列表、按钮、状态展示，以及启动/停止系统 SSH 进程。
