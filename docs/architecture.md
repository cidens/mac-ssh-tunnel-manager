...
SSH Config 模式只保存 `sshConfigName`，由 `~/.ssh/config` 中的 Host 条目负责声明 `LocalForward`、`RemoteForward` 或 `DynamicForward`：
...
- SSH Config 模式会检查 `ssh -G` 解析出的本地、远端和动态转发监听地址，并使用相应的风险确认。
...
9. 添加 SSH Config 模式隧道，确认配置名不存在或没有任何转发指令时保存失败；配置有效 SSH Config 后启动，确认可打开 `openURL`。
...
19. 分别选择名称、运行状态和最近使用排序，确认两秒状态刷新不会打乱同组配置。
...
- 只支持 macOS。
- 手工配置每条只支持一条本地、远程或动态 SOCKS 规则；SSH Config 引用会按 Host 中解析出的全部转发指令运行，不提供应用内逐条编辑。
- 不自动编辑 `~/.ssh/config`。
- 不做登录项自启动。
