...
本地监听探测每轮只运行一次固定的 `/usr/sbin/lsof -nP -iTCP -sTCP:LISTEN`，再用同一份输出判断全部本地和 SOCKS 端点，规则数量不会增加子进程数量。存在运行意图的本地监听时每 2 秒自动检查；全部停止时每 30 秒检查一次外部端口占用。启动、停止、保存、导入和用户手工刷新仍立即触发检查，且同一时刻最多存在一个后台状态探测任务。
...
19. 分别选择名称、运行状态和最近使用排序，确认运行中 2 秒刷新及全部停止时 30 秒刷新均不会打乱同组配置。
