...
- `TunnelMenuView.swift`：菜单栏窗口 UI，包括搜索、筛选、排序、添加、编辑、启动、停止、打开 URL 和删除。
...
- `id`：隧道唯一标识。
- `mode`：`localForward`、`remoteForward`、`dynamicForward` 或 `sshConfig`。
- `name`：界面显示名称。
- `openURL`：可选的本地访问 URL。
- `sshHost`、`localHost`、`localPort`、`remoteHost`、`remotePort`：手动转发模式使用。
- `sshHost`、`remoteHost`、`remotePort`、`localHost`、`localPort`：远程转发模式分别表示 SSH Host、远端监听和本地目标。
...
旧版 JSON 没有 `mode` 字段时，默认按 `localForward` 解码。没有标签、收藏、手工顺序、最近使用时间和自动重连字段时使用兼容默认值；所有配置均缺少 `manualOrder` 时，按原 JSON 数组顺序建立初始手工顺序。
...
远程转发和 SSH Config 模式不使用本地端口探测，因此运行中按应用进程状态显示 `Running`；如果 SSH 因转发失败退出，stderr 会显示在隧道卡片中。
...
- 手动转发、远程转发、动态 SOCKS 和 SSH Config 四种命令生成。
...
- 远程转发模式显示 SSH Host、远端监听和本地目标字段，不显示 SSH Config 字段。
...
16. 添加带标签的多条脱敏配置，组合使用搜索、标签和仅收藏筛选，确认隐藏配置的运行状态不受影响。
17. 选择手工排序，在无筛选条件下用上下箭头移动配置；重启应用后确认顺序保持。
18. 分别选择名称、运行状态和最近使用排序，确认两秒状态刷新不会打乱同组配置。
19. 使用多个长标签检查横向滚动、结果数量和清除入口均可操作。
20. 添加远程转发并保持远端监听为 `localhost`，确认保存时不显示暴露警告。
21. 把远端监听改为 `*`，确认警告包含当前地址、端口和 `GatewayPorts`，取消后配置不保存，确认后才保存。
22. 在受控 SSH 服务器启动远程转发，确认服务器端端口可访问 Mac 本地目标；停止隧道后确认端口失效。
23. 检查首次保存远程转发前生成的 `tunnels.json.pre-remote-forward.bak`，按恢复步骤确认旧配置可无损读取。
24. 为一条脱敏配置启用自动重连，制造可恢复连接中断，确认重试间隔依次为 2、5、10、30、60 秒且没有高速循环。
25. 在等待重连、正在连接和等待网络阶段分别点击“停止”，确认状态变为“已停止”且之后不再启动。
26. 启动开启自动重连的配置后断网或让 Mac 睡眠，确认期间不重试；恢复后等待 2 秒且只启动一次。
27. 分别制造认证失败、Host Key 失败和端口冲突，确认进入失败状态、保留脱敏错误且不自动重试。
...
- 只支持 macOS。
- 每条配置只支持一条本地、远程、动态 SOCKS 或 SSH Config 转发规则，不支持多规则 Profile。
- 不自动编辑 `~/.ssh/config`。
- 不做登录项自启动。
