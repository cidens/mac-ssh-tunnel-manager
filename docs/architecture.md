...
标签直接作为动态分组，不增加“场景”模型、持久化字段或配置迁移。选中标签时，`TagGroupSnapshot` 从完整配置集合按大小写及变音符号不敏感规则匹配成员，并按手工顺序输出；搜索、收藏和排序只影响列表展示，不改变批量操作范围。一次快照同时生成互斥的运行、连接中或等待、失败和停止数量，避免为每个状态或标签重复全量扫描。

批量启动先对标签全部成员建立稳定 UUID 引用，再通过最多 4 个并发任务执行规则校验、风险确认检查、SSH Config 转发解析和监听冲突预检。每个批次把全部已启用的本地监听规则建立为一次性的所有者感知索引，并至多读取一次系统监听快照；成员预检只查询共享索引，不再为每个成员重复扫描全部连接组。索引按端口保存首条内联记录，仅在同端口存在更多监听时分配附加数组，空间复杂度为 `O(R)`，其中 `R` 最多为 20000 条规则。启动请求仍按手工顺序提交；已经运行、没有启用规则、风险未确认、预检失败或操作期间被删除及修改的成员会跳过并记录原因，单条失败不阻塞后续成员。配置在预检后变化时会按 UUID 重新解析和复检，手工停止或批量停止会使尚未提交的旧启动请求失效。

批量停止覆盖托管进程、正在连接、等待网络和等待重连状态，并取消同一时间仍在进行的批量启动。进程终止最多并发 4 个并在后台等待，结果按原手工顺序汇总。批量操作通过运行代次拒绝取消后的过期结果；父任务取消会继续传递到监听快照和 `ssh -G` 子进程，应用终止时不会等待它们自然超时。成员引用、预检结果和结果汇总均受 1000 个连接组上限约束，不保存历史批次；连接删除并成功写盘后同时清理手工停止代次。最近使用时间在批次中合并写盘，避免为每个成员重复序列化完整配置。
...
发布包隔离验收可显式设置 `SSH_TUNNEL_MANAGER_APPLICATION_SUPPORT_DIRECTORY`，让隧道配置、快捷键和通知设置统一使用指定的应用支持目录。覆盖值必须是绝对路径；空值或相对路径会被忽略。普通 Finder、Spotlight、登录项和命令行启动不设置该变量时，仍使用系统用户应用支持目录。
...
- 隔离验收应用支持目录只接受显式绝对路径，空值和相对路径回退到系统用户目录。
- 标签动态分组的大小写不敏感匹配、手工顺序、互斥状态汇总，以及筛选不改变批量范围。
- 批量启动和停止的 4 路并发上限、稳定结果顺序、单条失败隔离、一次监听快照、父子任务取消传播和过期结果拒绝。
- 1000 个连接组、每组 10 个标签的聚合性能，1000 个连接组、每组 20 条规则的预检索引和同端口写时复制防退化性能，100 个成员的批量调度性能，以及慢预检不阻塞主线程。
