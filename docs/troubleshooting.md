...
健康状态与 SSH 进程状态相互独立。SSH 已连接只表示转发进程仍在运行；连续 3 次规则级检查失败后，应用会把该规则标记为异常，但不会停止进程或触发自动重连。

排查顺序：

- TCP 检查：确认规则的本地监听地址和端口仍可连接，并检查是否有其他进程抢占端口。
- HTTP/HTTPS 检查：确认健康检查 URL 指向当前规则的本地监听端口，服务返回 `200...399`，HTTPS 证书能通过 macOS 系统信任校验。
- SOCKS5 检查：确认该规则为动态 SOCKS，并且监听支持 SOCKS5 无认证方法；检查不会访问任何外部目标。
- 在“连接详情”中查看上次检查时间和失败类别。一次成功检查会立即恢复健康状态。
- 断网或系统睡眠期间检查会暂停，恢复后会错峰重新开始；此时短暂显示“等待健康检查”属于正常现象。

如果不需要应用层检查，可编辑规则并关闭“启用连接健康检查”。远程转发和 SSH Config 引用首版只显示 SSH 进程状态，不提供端到端健康检查。
